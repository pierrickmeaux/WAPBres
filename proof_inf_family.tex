...
\begin{equation*}
    \begin{split}
        ...
        &2\sum_{y_i \in \vecy_{\{1,2\}}} (y_i) \cdot 2^{i_1 + i_2} + 2\sum_{y_i \in \vecy_{\{1,3\}}} (y_i) \cdot 2^{i_1 + i_3} + \ldots + 2\sum_{y_i \in \vecy_{\{t-2, t-1\}}} (y_i) \cdot 2^{i_{t-2} + i_{t-1}} = 0.
    \end{split}
\end{equation*}
We use the previously defined notations and knowledge that $s_1 = s_2 = \ldots = s_t = 0$
to simplify it into
\begin{equation}\label{eq:lin_t_row2}
    2s_{12} \cdot 2^{i_1 + i_2} + 2s_{13} \cdot 2^{i_1 + i_3} + \ldots + 2s_{(t-2)(t-1)} \cdot 2^{i_{t-2} + i_{t-1}} = 0.
\end{equation}
It is clear that any $s_j$ can take for values any even number or its opposite
