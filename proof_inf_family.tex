...
\begin{equation}\label{eq:vand_t}
        \begin{bmatrix}
            1 & 1 & 1 & 1 & \ldots & 1\\
            0 &  2^{i_1} & 2^{i_2} & (2^{i_1} + 2^{i_2}) & \ldots & (2^{i_1} + \cdots + 2^{i_t})\\
            \vdots & \vdots & \vdots & \vdots &  & \vdots\\
            0 & 2^{(t-1)i_1} & 2^{(t-1)i_2} & (2^{i_1} + 2^{i_2})^{t-1} & \ldots & (2^{i_1} + \cdots + 2^{i_t})^{t-1}\\
        \end{bmatrix}
...
\end{equation}
Since $\vecy$ is balanced, it is clear that any $s_j$ can take for values any
...
because the gap between any two $i_j$ is at least $t$, there doesn't exist
...
\begin{equation*}
    \begin{split}
        &\left(\sum_{y_i \in \vecy_{\{1\}}} y_i\right) \cdot 2^{2 i_1} + \left(\sum_{y_i \in \vecy_{\{2\}}} y_i\right) \cdot 2^{ 2 i_2} + \cdots + \left(\sum_{y_i \in \vecy_{\{t\}}} y_i\right) \cdot 2^{ 2 i_t} +\\
...
\begin{equation}\label{eq:lin_t_row2}
    2s_{1,2} \cdot 2^{i_1 + i_2} + 2s_{1,3} \cdot 2^{i_1 + i_3} + \cdots + 2s_{t-1,t} \cdot 2^{i_{t-1} + i_{t}} = 0.
\end{equation}
...
the same argument to prove there does not exist any $s_{i,j} \neq 0$ satisfying
...
\begin{equation*}
    \begin{split}
        ...
        \mu_{1,2} s_{1,2} 2^{i_1 + i_2} + \cdots + \mu_{1,2} s_{t-1,t}+ 2^{i_{t-1} + i_{t}} &+\\
        \cdots &+\\
        \mu_{1,2,\ldots, r} s_{1,2,\ldots, r} 2^{i_1 + i_2 + \cdots + i_r} + \cdots + \mu_{1,2,\ldots, r} s_{t-r,t-r+1,\cdots,t} \cdot 2^{i_{t-r} + i_{t-r+1} + \cdots + i_{t}} &= 0.
    \end{split}
\end{equation*}

We know from Row $r-1$ that $s_1, \ldots, s_t, s_{1,2}, \ldots, s_{t-1,t}, \ldots, s_{t-(r-1),\ldots,t-1,t} = 0$.
...
condition that $[i_1, \ldots, i_t]$ is a $t$-gap $B_{t-1}$ sequence and bring
the same argument to prove there does not exist any $s_j \neq 0$ satisfying
