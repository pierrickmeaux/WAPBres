...
\begin{proposition} Let $n\in \N^*$ with $\w(n)=2$. Then there exist WAPB functions $f$ with $\corr(f)=1$, and the 
...
\begin{proposition}Let $n\in \N^*$ with $n=2^{i_1}+\cdots + 2^{i_t}$ with $i_1>\cdots > i_t$ and $t>3$.
	Furthermore, then there exist WAPB functions $f$ with $\corr(f)=t-1$.
	then the 
	restricted Walsh transforms $\rwv{f}$ are uniquely given by 
	\[
	\wtk{f}{\ell} = (-1)^{\w(\ell)}, \forall \, \ell \preceq n,
