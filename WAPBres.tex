...
\mgnote{Isn't there a notation inconsistency in Lemma 2? It seems the multinomial coefficient should be $\frac{u!}{u_{j_1}!\ldots u_{j_s}!}$. Same in the expansion of $x_{j_1}^{u_{j_1}}\ldots$}
...
\begin{proposition}Let $n\in \N^*$ with $n=2^{i_1}+2^{i_2}+\cdots +2^{i_t}$ with $i_1<i_2<\cdots i_t$ and $t>3$.
	...
	Then any WAPB function $f$ has $\corr(f)\leq t-1$. Furthermore, the equality
	is reached if and only if the restricted Walsh transforms $\rwv{f}$ is given
	by

	\mgnote{Shouldn't it be "... any WAPB function \textbf{in n variables} $f$ has $\corr(f)\leq t-1$..." or is it implicit? }
	$$
	\wtk{f}{\ell} = \wtk{f}{0} \cdot (-1)^{\w(\ell)}, \forall \, \ell \preceq n.
	$$
\end{proposition}
