...
Therefore,  Prop.~\ref{prop:WalsCorr} can be transformed as follows. 

\begin{proposition}\label{prop_t_corrector_Vandemonde_matrix}
	Let $n\in \N^*$ and $t\in \N$ such that $t\le n$. 
	A Boolean function $f\in \Bn{n}$ has $\corr(f) = t$ if and only if its restricted Walsh transform vector $\rwv{f}$ satisfies
\begin{equation}\label{Eq_RWT}
\matV^{(t+1)}
\rwv{f} = \textbf{0}_{t+1}^\intercal \text{ and } \matV^{(t+2)}\rwv{f} \neq  \textbf{0}_{t+2}^\intercal,
\end{equation}
where $\matV^{(r)}$ for $r\in \{t+1, t+2\}$ is the submatrix formed by the leading $r$-rows of the Vandemonde matrix $\matV$, namely, 
\[
\matV^{(r)} = \begin{bmatrix}
1 & 1 & 1 & \cdots & 1 \\
0 & 1 & 2 & \cdots & n \\
&  &  \vdots & & \vdots  \\
0 & 1 & 2^{r-1} & \cdots & n^{r-1}
\end{bmatrix}.
\]
...
\begin{proposition}Let $n\in \N^*$ with $n=2^{i_1}+2^{i_2}+\cdots +2^{i_t}$ with $i_1<i_2<\cdots i_t$ and $t>3$.
	Suppose $\lambda_n := \{i_t-i_{t-1}, \dots, i_{3}-i_2, i_2-i_1 \}$ satisfies $\lambda_n\geq t$.
	Then any WAPB function $f$ has $\corr(f)\leq t-1$.
    Furthermore, the equality is reached if and only
	the
