...
Since the slices and the hyperplanes corresponding to affine relations are not related, one might expect the WAPB function to behave like random balanced functions relatively to the the resilience. 
...
More generally, WAPB functions have a low resilience order (the maximum value of $t$ such that the function is $t$-resilient). We can exhibit $n$-variable WAPB functions with a resilient order no more than logarithmic in $n$, and 
