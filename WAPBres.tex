...
\mgnote{We should define Krawtchouk polynomials to take value 0 when $k < 0$.
\begin{equation*}
	\kraw{k}{\ell}{n} =
	\begin{cases}
		\sum_{j=0}^{k} (-1)^j \binom{\ell}{j} \binom{n-\ell}{k-j} & 0 \leq k \leq n\\
		0 & k < 0\\
	\end{cases}
\end{equation*}}

\clnote{ Martin: I believe the comment on ${m\choose r} = 0$ for $r<0$ covers the cases where $k<0$.}
	...
	where we take the binomial coefficient ${m\choose r} = 0$ when $m<r$ and $r<0$ by convention.
		Krawtchouk polynomials are characterized by the generating series: $$ \displaystyle (1+z)^{n-\ell} (1-z)^\ell=\sum_{k=0}^\infty \kraw{k}{\ell}{n} z^k.$$
\end{definition}
...
\mgnote{Don't you think there is ambiguity between the Krawtchouk matrix defined here and its transposed used in Theorem \ref{Th_K_V_matrices}?}
\end{definition}

Denote by $\matK^{(t+1)}$ the submatrix formed by the leading $(t+1)$ rows of $\matK$.
By Proposition~\ref{prop:WalsCorr} we see that $f$ is a $t$-corrector if and only if:
...
\begin{theorem}\label{Th_K_V_matrices}
	...
	Let
	\[
	\alpha_k = -\frac{k}{2}, \beta_k = \frac{n}{2}, \gamma_k = -\frac{n-k}{2}, \quad 0\leq k\leq n,
	\]
	...
	\end{equation}
	where $\matM_{r,[0,r]} = [\matM_{r,0},\dots, \matM_{r,r}]$ denotes the leading $(r+1)$ entries in the $r$-th row of $M$.
...
Therefore,  a Boolean function $f$ is a $t$-corrector if and only its restricted Walsh transform $\rwv{f}$
satisfies the following system of equations:
\begin{equation}\label{Eq_Vandemonde_system}
\matV^{(t+1)} \cdot \rwv{f} = \mathbf{0}_{t+1}^\intercal.
\end{equation}
...
\begin{theorem}\label{Th-corrector-by-Vandemonde}
	Let $\matW$ denote the submatrix of $\matV^{(t+1)}$ formed by the $\ell$-th column in $\matV^{(t+1)}$ with $\ell \preceq n$.
...
\begin{proof}
	...
	By~\eqref{Eq_Vandemonde_system} we see that $\corr(f) = 1$ if and only if
	\[
	...
	%
	%	Using Proposition~\ref{prop:WalsCorr} and Property~\ref{prop:Kr}, for $n=2^m$ we have:
...
\begin{proposition} Let $n\in \N^*$ with $\w(n)=2$. Then there exists a WAPB functions $f$ with $\corr(f)=1$, and the
...
\begin{proof}
	...
	\[
	\mathbf{y} = \pm (1, -1,-1, 1).
	\] This is equivalent to saying that $\wtk{f}{\ell} = c (-1)^{\w(\ell)}$ for any $\preceq n$, where $c$ equals either $1$ or $-1$.
	Furthermore, these solutions do not satisfy \eqref{Eq_t=2}, which implies that there exists no WAPB function $f$ with $\corr(f) = 2$.
\end{proof}

\begin{proposition} Let $n\in \N^*$ with $\w(n)=3$. Then there exist a WAPB functions $f$ with $\corr(f)=2$, and the
...
\end{proposition}
\begin{proof}
	\clnote{Martin: please complete this proof similarly.}
\end{proof}
...
\begin{lemma}\label{Represention_Equations}
	...
	\sum_{s=1}^t\sum\limits_{\substack{\{j_1,\dots, j_s\} \subseteq [t] \\ u_{j_1}+\cdots+ u_{j_s}=u \\  u_{j_1},\dots,u_{j_s}\geq 1}}  \frac{u!}{u_1!\cdots u_s!} \left(  \sum_{\substack{\{i_{j_1},\dots, i_{j_s}\} \subseteq supp(\ell)   \\ \ell\preceq n }}  y_{\ell} \right)  x_{j_1}^{u_1}\dots x_{j_s}^{u_{j_s}},
...
\begin{proposition}Let $n\in \N^*$ with $n=2^{i_1}+\cdots + 2^{i_t}$ with $i_1>\cdots > i_t$ and $t>3$.
	Furthermore,
	suppose $\lambda_n := \{i_1-i_2, i_2-i_3, dots, i_{t-1}-i_t \}$ satisfies $\lambda_n\geq t$,
	then there exists a WAPB function $f$ with $\corr(f)=t-1$,
	and
	the
	restricted Walsh transforms $\rwv{f}$ is given by
	\[
	\wtk{f}{\ell} = (-1)^{\w(\ell)}, \forall \, \ell \preceq n,
	\]
   and there exists no WAPB function $f$ with $\corr(f)=t$.
\end{proposition}
\begin{proof}
	\clnote{Martin: please try to give a proof by following the proof development for Prop. 4}
\end{proof}
...
\mgnote{I propose the proof for the relation between $\mathbf{K}$ and $\mathbf{V}$ below. Every feedback is welcome.}
\section{Relation between the Krawtchouk matrix and the Vandermonde matrix.}
\input{kraw_vand_matrices_eq.tex}
