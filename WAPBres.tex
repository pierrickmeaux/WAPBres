...
\begin{theorem}\label{Th_K_V_matrices}
	Let $\matK$ be the Krawtchouk matrix with $\matK_{k,\ell} = K_k(\ell, n)$ and $\matV$ be the Vandemonde matrix with $V_{k,\ell} =\ell^k$, where $k,\ell$ with $0\leq k,\ell\leq n$ index the rows and columns, respectively.
	Let
	\[
