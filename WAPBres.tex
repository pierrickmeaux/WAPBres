...
\begin{proposition} Let $n\in \N^*$ with $\w(n)=2$. Then any WAPB function $f$ has $\corr(f)\leq 1$.
...
%\end{proposition}
\begin{proposition} Let $n\in \N^*$ with $\w(n)=3$. Then any WAPB function $f$ has $\corr(f)\leq 2$.
	Furthermore, the equality is reached if and only if the restricted Walsh vector $\rwv{f}$ is given by
	\[
	\wtk{f}{\ell} =   \wtk{f}{0} \cdot (-1)^{\w(\ell)}, \forall \, \ell \preceq n.
	\]
