...
WPB Functions obtained by comparing the weights of two halves of the input achieve optimal AI~\cite{CC:TangLiu19,BFA:MesSuLi21}. Comparing the two halves using total orders lead to WAPB functions for all $n$~\cite{SAC:Meaux24}.
...
This is particularly intriguing since what characterize a WAPB function is related to the balancedness, and balancedness on all the parts of a particular partition.
...
\pmnote{add reference to correctors}

To analyze the resilience of WAPB functions, we adopt a perspective that is uncommon in Boolean function used for cryptography, we study their properties as correctors.
A $t$-corrector is a function whose sum over a slice of Walsh coefficients is null, for all slice of index from $0$ to $t$.
We show that the correction order of a Boolean function is tightly connected to a system of linear equations involving the restricted Walsh transforms on the slices of the function.
...
A key component of our analysis is an algebraic connection between Krawtchouk matrices and Vandermonde matrices.
By analysing the restricted Walsh equations satisfied by WAPB functions, we show that determining the maximum correction (and hence an upper bound on the resilience) order of such functions reduces to a problem closely related to the classical Prouhet-Tarry-Escott problem.
...
Since there exist WAPB functions with resilience $\w(n)-1$, our bound is tight for the infinite families of $n$ where we could prove it, and for all $n$ general if the conjecture is true.
...
\mgnote{
Recall that a set of integers $A$ is said to be a \emph{Sidon set} if all sums
$a_1 + a_2,~ a_i \in A$. are all distinct (up to commutativity). The work of
\cite{genralized_sidon_sets} defines a set $A$ to be a $g$-Sidon set if $r(x) \leq g$
for all $x$, with $r(x)$ the representation function of $A$ with $r(x) = |\{(a_1, a_2): a_i \in A, a_1 + a_2 = x\}|$.
From the definition of $g$-Sidon set, we see that a $g$-Sidon set is also a
$k$-Sidon set for $2 \leq k \leq g$ (since any two sums of $k$ elements can be
seen as two sums of $h$ elements where the last $h-k$ terms are exactly the same).

Proceeds on to $m$-gap.

Recall that a superincreasing sequence $s_1, s_2, \ldots$ satisfies
$$
s_{n+1} > \sum_{i=1}^n s_i
$$
for all $n$. A direct property that follows is the uniqueness of every subset
sum of this sequence. As such, any superincreasing sequence of $g$ elements
constitutes a $g$-Sidon set.
}
