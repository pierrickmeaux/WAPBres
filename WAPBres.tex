...
designed to meet its specific criteria. Among promising candidates for such
constructions stand out the FLIP cipher that employs Boolean functions defined on specific subsets of $\mathbb{F}_2^n$. In this
article, we study a family of such functions called Weightwise (Almost)
Perfectly Balanced (W(A)PB) functions, that have the property of being balanced
on each of the considered subsets. While W(A)PB functions have been of great
...
%We also reduce the
%problem of determining the correction order of a W(A)PB function to a weak
%instance of the Prouhet-Tarry-Escott problem. 
...
Since $2017$, many works have introduced new constructions of WPB or WAPB functions, studying their global and restricted cryptographic parameters.
%—algebraic degree, Algebraic Immunity (AI), nonlinearity—as well as restricted parameters such as weightwise nonlinearity and weightwise algebraic immunity.
...
WPB Functions obtained by comparing the weights of two halves of the input achieve optimal algebraic immunity~\cite{CC:TangLiu19,BFA:MesSuLi21}.
Comparing the two halves using total orders lead to WAPB functions for all $n$~\cite{SAC:Meaux24}.
...
Secondary constructions have also been explored, such as direct sums~\cite{DAM:ZhuSu22}, Siegenthaler constructions combined with symmetric functions~\cite{INDO:GinMea22} and $t$-concatenation~\cite{eprint:IsiRodSeh25}.
Moreover, several works have modified the support of non-WPB functions to %analyze the distribution of cryptographic parameters
provide W(A)PB functions with bounded parameters for
the weightwise nonlinearity~\cite{DAM:GinMea22}, the global nonlinearity~\cite{C2SI:GinMea23}, or the algebraic immunity~\cite{LC:GinMea23}.
...
Additional constructions appear in~\cite{BFA:DalMal23,AMC:DalMal23,eprint:IsiRodSeh25}.
...
Generalizing the notion of balancedness, a function is said $t$-resilient if it is uncorrelated to all affine functions combining at most $t$ variables.
The resilience has been introduced to estimate the complexity of correlation attacks on streamciphers~\cite{Siegenthaler84}.
Previous studies on WAPB functions have characterized their nonlinearity~\cite{C2SI:GinMea23}, restricted nonlinearity~\cite{DAM:GinMea22}, algebraic immunity (both global~\cite{LC:GinMea23} and weightwise~\cite{CANS:BonMea25}), algebraic degree~\cite{CCDS:GinMea24}, yet no general result on the resilience of WAPB functions had been established.


This is particularly intriguing since what characterizes a WAPB function is related to the balancedness, and balancedness on all the parts of a particular partition.
Since the slices and the hyperplanes corresponding to affine relations are not related, one might expect the WAPB function to behave like random balanced functions relatively to the resilience.
However, our results show that this is not the case.
Even the simplest case, WPB functions, provides a stark example: although they are balanced on every slice, they cannot be $1$-resilient.
More generally, WAPB functions have a low resilience order (the maximum value of $t$ such that the function is $t$-resilient).
We can exhibit $n$-variable WAPB functions with a resilient order no more than logarithmic in $n$, and
demonstrate for infinite values of $n$ that no WAPB function can be $\log_2(n)$-resilient.

\medskip

To analyze the resilience of WAPB functions, we adopt a perspective that is uncommon in Boolean function used for cryptography, we study their properties as correctors \eg ~\cite{IEEE:Lacharme09,DCC:LWZS24,DCC:HuZhangWu26}.
A $t$-corrector is a function whose sum over a slice of Walsh coefficients is null, for all slice of index from $0$ to $t$.
We show (Theorem~\ref{thm:WalsCorr}) that the correction order of a Boolean function is tightly connected to a system of linear equations involving the restricted Walsh transforms on the slices of the function.
For WAPB functions, these equations are highly simplified since the restricted Walsh transform of these functions takes only the value $-1$, $0$ or $1$.
The link between correctors and restricted Walsh transform provides a general framework to study resilience through corrector properties, and is of independent interest for understanding the number and structure of $t$-corrector or $t$-resilient Boolean functions.


A key component of our analysis is an algebraic connection between Krawtchouk matrices and Vandermonde matrices (Theorem~\ref{Th_K_V_matrices}).
It allows us to rewrite the problem of maximum corrector order based on properties of a Krawtchouk matrix as a problem on properties of a, simpler to study, Vandermonde matrix.
By analyzing the restricted Walsh equations satisfied by WAPB functions, we show (Theorem~\ref{thm:t_corrector_Vandemonde_matrix}) that determining the maximum correction (and hence an upper bound on the resilience) order of such functions reduces to a problem closely related to the Prouhet-Tarry-Escott problem \eg \cite{Borwein2002}.

%This connection allows us to derive tight bounds on the correction order of WAPB functions in some cases.
Our main result (Proposition~\ref{prop:infFamily}) is that for infinitely many values of $n$, no WAPB function has correction order greater than or equal to the Hamming weight of $n$.
...
Since there exist WAPB functions with resilience $\w(n)-1$, our bound is tight for the infinite families of $n$ where we could prove it, and for all $n$ in general if the conjecture is true.


%\pmnote{Re-build a more detailed contribution part with links to the sections}
...
We give the necessary preliminaries here and refer to \eg ~\cite[Ch.5, \S 7]{book:MacSlo78} for more details.
...
Following from Property~\ref{prop:WAPBWalsh}, we reformulate WAPB functions of correction order $t$ in terms of Krawtchouk polynomials.
...
The following theorem provides an interesting connection between the Krawtchouk matrix and the Vandemonde matrix. This connection facilitates the study of the above equation.
	...
	\pmnote{Suggestion: In the proof, add the "M is triangular" argument at the initialization step and induction.}
...
\alpha_k = -\frac{n-k}{2}, \beta_k = \frac{n}{2}, 	\gamma_k = -\frac{k}{2}, \quad 0\leq k\leq n,
	...
	\pmnote{Suggestion: we could write $V_{n+1}$ in the following equation or mention somewhere its size (for easy checking)}
...
\end{theorem}
\begin{proof}
	Property~\ref{prop:KrDMS} is crucial for us to derive the lower triangular matrix $\matM$ satisfying $\mathbf{M} \cdot \mathbf{K} = \mathbf{V}$.
	...
	Property~\ref{prop:KrDMS} can be equivalent stated as
	\begin{equation*}
	...
	To do so, we expand $\ell^1$ using Eq.~\eqref{eq:prop3sym} as follows:
	\begin{equation*}
	\begin{split}
	...
	Similarly, we expand $\ell^2$ using Eq.~\eqref{eq:prop3sym} as follows:
	\begin{equation*}
	\begin{split}
	...
	\end{split}
	\end{equation*}
	which implies $\matM_{r,[0,r]} = \matM_{r-1,[0,r]}\cdot \matH_r$ and $\matLead{M}{r}{n} \cdot \matK = \matLead{V}{r}{n}$. The desired statement follows.
\end{proof}

Denote by $\matM_{[0,t],[0,t]}$ the $(t+1)$-order submatrix formed by the leading $(t+1)$ rows and columns of $\matM$.
The matrix $\matM_{[0,t],[0,t]}$ is invertible since the matrix $\matM$ is an invertible lower triangular matrix by Theorem~\ref{Th_K_V_matrices}.
...
The characterization of $t$-corrector in terms of the Vandemonde matrix above
will be used to study the existence of WPB functions and WAPB functions of
certain order $t$ in the next section.

%\pmnote{Here add the connection with the PTE problem}
...
\begin{proof}
	...
	It can be verified that the solutions of \eqref{Eq_t=1} are given by
	\[
	\mathbf{y} = \pm (1, -1,-1, 1).
	\] This is equivalent to saying that $\wtk{f}{\ell} = \wtk{f}{0} \cdot (-1)^{\w(\ell)}$ for any $\ell \preceq n$.
	Furthermore, these solutions do not satisfy \eqref{Eq_t=2}, which implies
	that there exists no WAPB function $f$ with $\corr(f) = 2$ when $\w(n) = 2$.
\end{proof}
...
\subsubsection{Corrector order of an infinite family of $n$-variable WAPB functions.}
Recall that a sequence of integers $A$ is said to be a $B_h$ sequence if the
sums of $h$ integers from $A$, i.e.,
$$
s = a_1 + \cdots + a_h, ~a_1\leq \cdots \leq a_h, ~a_i \in A
$$
where the $a_i$s are all distinct \eg \cite{cilleruelo2017}. From the definition of $B_h$ sequence,
we see that a $B_h$ sequence is also a $B_k$ set for integers $k$ with
$2 \leq k < h$ (since any two sums of $k$ elements can be seen as two sums of
$h$ elements where the last $h-k$ terms are exactly the same).

\pmnote{I think I am missing some constraints in the definition, $A= \{1,2,3,4,5\}$ is a $B_5$ sequence, also a $B_1$ and $B_4$ set but not $B_2$ or $B_3$.
This example is a counterexample of the part in parenthesis.

}


Furthermore, we shall call a sequence $A$ an $m$-gap $B_h$ sequence if $A$ is a $B_h$ sequence and
\begin{equation}\label{eq:m-gap_sidon}
...
\pmnote{Regarding the equation above, relaxing the definition to $\ge m$ rather than $=m$ might be easier later on, what do you think?}

Given a $B_h$ sequence $A$, it is easily seen that the set $mA = \{ m \cdot a ~: ~a \in A \}$ is an $m$-gap $B_h$ sequence.

Finally, recall that a superincreasing sequence $s_1, s_2, \ldots$ satisfies
$$
s_{n+1} > \sum_{i=1}^n s_i
$$
for all $n$. A direct property that follows is the uniqueness of every subset
sum of this sequence. As such, any superincreasing sequence of $h$ elements
also constitutes a $B_h$ sequence.
...
\pmnote{For this (very elegant) result, I think it would benefit the paper to present it as a theorem.

Regarding the proof, for me it shows that the coefficients $s_i$ need all to be $0$, but it misses the parts showing that:

1) it gives an impossibility for $t$-correction,

2) the $\wtk{f}{0} (-1)^\w(\ell)$ are the only one satisfying it.

}



\begin{theorem}\label{prop:infFamily}Let $n\in \N^*$ with $n=2^{i_1}+2^{i_2}+\cdots +2^{i_t}$ with $t>3$ and $i_1<\dots < i_t$.
...
Based on the different theoretic results of this section such as Proposition~\ref{prop:infFamily} and experimental results (Section~\ref{sec:exp}), we introduce the following conjecture on the corrector and resilience order of WAPB functions.
...
%	Increasing the Hamming weight of one doubles the number of non null restricted Walsh transform coefficients and increases by one the degree of the last polynomial to consider, that is why we do not extend the proof strategy for $n$ of higher Hamming weights.
	Regarding the resilience upper bound of $\w(n)-1$, we know that for all $n$ there exist WAPB functions with resilience order $\w(n)-1$:
	when we write $n$ as $n=2^{i_1}+2^{i_2}+\dots + 2^{i_t}$ where $i_1<\dots<i_{t}$,
	the direct sum of WPB functions in $2^{i_s}$ variables~\cite{DAM:ZhuSu22} have resilience order exactly $\w(n)-1$.
	Such WAPB functions are exactly characterized in the conjecture, namely, their Walsh transforms $\rwv{f}$ are given by 
	\(\wtk{f}{\ell} = (-1)^{\w(\ell)}, \forall \, \ell \preceq n\). 
\end{remark}

\begin{remark}
	Recall that the Prouhet-Tarry-Escott problem~\cite{wright_prouhets_1959}
	asks for two disjoint multi-sets $S_1$ and $S_2$ of the same number of integers
	satisfying 
	\[
	\sum_{a\in S_0} a^s = 	\sum_{b\in S_1}b^s, \quad  s = 0, 1, \dots, r.
	\]
	The Prouhet-Tarry-Escott problem can be phrased as a $r$-regular partition of an integer set ~\cite{bolker_prouhettarryescott_2016}. 
	More concretely, for a set $S$ of even number of integers, a partition $\{S_0,S_1\}$ of
	$S$, i.e., $S_0\cap S_1 = \emptyset$ and $S_0\cup S_1=S$, is $r$-regular  if 
	\[
	\sum_{a\in S_0} a^s = 	\sum_{b\in S_1}b^s, \quad  s = 0, 1, \dots, r.
	\]
	 When the set $S=\{0,1,\dots, 2^{t}-1\}$, the Prouhet-Thue–Morse sequence~\cite{allouche_ubiquitous_1999} gives 
	 a $(t-1)$-regular partition of $S$ as  
	$$S_i = \{ 0\leq a<2^{t}-1: \w(a)\equiv i \mod{2}\}, i= 1,2.$$ 
	Furthermore, it was conjectured in~\cite[Conjecture 5.3]{bolker_prouhettarryescott_2016} that 
	the above partition $\{S_0, S_1\}$ is the unique $(t-1)$-regular partition of $S=\{0,1,\dots, 2^{t}-1\}$.
	One can easily verify that this partition is not $t$-regular.
	
	Observe that Conjecture~\ref{conj:wapb-corr} is a generalized version of the second part of Conjecture 5.3 in~\cite{bolker_prouhettarryescott_2016}.
	For a positive integer $n$ of the form $n=2^{i_1}+2^{i_2}+\dots + 2^{i_t}$ with $i_1<\dots<i_{t}$,  if in Corollary~\ref{Th-corrector-by-Vandemonde}
	the system of equations \(	\matW \cdot \mathbf{y} = \mathbf{0}_{{t+1}}^{\intercal}\) 
	has a solution $\mathbf{y}=(y_0,y_1,\dots, y_{2^t-1})^\intercal \in \{\pm 1\}^{2^t}$, then it 
	can be used to derive 
	a partition $\{S_0,S_1\}$ of the set 
	$$
	S= \{a_12^{i_1} + a_22^{i_2} + \cdots + a_t2^{i_t}: a_1,a_2,\dots, a_t \in \F_2\},
	$$
%	$$S=\left\{ 0\leq k <n :  k \preceq n \right\} = \left\{\sum_{i=1}^{t} a_i2^{k_i}: a_i \in \F_2\right\}$$ 
	where $S_0,S_1$ is derived based on the sign of $y_a$, where $a=a_1+2a_2+\cdots + 2^{t-1}a_t$.
	In other words, the existence of a WAPB function $f$ with $\corr(f)=t$ implies 
	that the existence of a $t$-regular partition of the set $S$, which appears unlikely to occur.
