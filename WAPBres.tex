...
By analyzing the restricted Walsh equations satisfied by WAPB functions, we show that determining the maximum correction (and hence an upper bound on the resilience) order of such functions reduces to a problem closely related to the Prouhet-Tarry-Escott problem \eg \cite{Borwein2002} (Theorem~\ref{thm:t_corrector_Vandemonde_matrix}.

%This connection allows us to derive tight bounds on the correction order of WAPB functions in some cases.
Our main result (Proposition~\ref{prop:infFamily}) is that for infinitely many values of $n$, no WAPB function has correction order greater than the Hamming weight of $n$.
...
\subsubsection{Of an infinite family of $n$-variable WAPB functions.}
...
$$
are all distinct \cite{cilleruelo2017}. From the definition of $B_h$ sequence,
