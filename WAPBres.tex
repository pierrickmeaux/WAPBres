...
constructions stand Linear Feedback Shift Registers (LFSR) with a filtering
Boolean function defined on specific subsets of $\mathbb{F}_2^n$. In this
article, we study a specific family of such functions called Weightwise (Almost)
...
of W(A)PB functions, through their properties as correctors. We highlight a
close connection with the (restricted) Walsh transform and uncover algebraic
relations between Krawtchouk and Vandermonde matrices. We also reduce the
problem of determining the $t$-correction of a W(A)PB function to a weak
instance of the Prouhet-Tarry-Escott problem. Furthermore, we prove the
existence of an infinite family of W(A)PB functions in $n$ variables with
$t$-correction less than the Hamming weight of $n$. We generalize this
observation into a conjecture that it holds for all $n$, that we verified until
$n \leq 62$.
\end{abstract}
...
\begin{proposition}\label{prop:infFamily}Let $n\in \N^*$ with $n=2^{i_1}+2^{i_2}+\cdots +2^{i_t}$ with $t>3$ and $i_1<\dots < i_t$.
...
%	Increasing the Hamming weight of one doubles the number of non null restricted Walsh transform coefficients and increases by one the degree of the last polynomial to consider, that is why we do not extend the proof strategy for $n$ of higher Hamming weights.
	Regarding the resilience upper bound of $\w(n)-1$ in the conjecture, we know that for all $n$ there exist WAPB functions with resilience order $\w(n)-1$: for example the ones obtained by direct sum of WPB functions in $2^{k_i}$ variables~\cite{DAM:ZhuSu22} when we write $n$ as $n=\sum_{i=1}^{\w(n)} 2^{k_i}$ where $k_1<\dots<k_{\w(n)}$.
